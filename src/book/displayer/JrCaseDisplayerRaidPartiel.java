 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class JrCaseDisplayerRaidPartiel extends JrCaseDisplayer {
	public int getName() {
		return JrCaseDisplayerName.CASE_DISPLAYER_RAID_PARTIEL;
	}
	...
	public  boolean isVisible(int name,boolean editionMode) {
		switch(name) {
		case JrRegionName.REGION_PARTIEL : return false;
		...
		case JrRegionName.REGION_TOTAL : return true;
	...
	public void apply(Rectangle rect,JrRegions reg,boolean showIndex) {		
		...
		reg.set(JrRegionName.REGION_PARTIEL,x,y,pasx,pasy);		
		reg.set(JrRegionName.REGION_RAZ,x,y+pasy,pasx,pasy);
		reg.set(JrRegionName.REGION_APPROX,x,y+pasy+pasy,pasx,pasy);
		reg.set(JrRegionName.REGION_TOTAL,x+(pasx/2),y+(pasy*3)+pasy2,(pasx/2),rect.height-(pasy*3)-pasy2);
